...
import { fetchSellerOrderMonthlyAnalytics, fetchSellerRevenueAnalytics } from '../../redux/slice/sellerDashboard.slice';
...
const Dashboard = () => {
    ...
    const { monthlyAnalytics, revenueAnalytics } = useSelector((state) => state.sellerDashboard);
    ...
    useEffect(() => {
        ...
        dispatch(fetchSellerRevenueAnalytics())

        if (user?._id) {
            dispatch(getAllProducts({ seller: user._id }))
        }
    }, [dispatch, user])

    useEffect(() => {
        dispatch(fetchSellerOrderMonthlyAnalytics(selectedYear));
