...
import { FiArrowUp, FiArrowDown, FiEye, FiEdit2, FiTrash2, FiSearch, FiChevronLeft, FiChevronRight, FiDownload } from "react-icons/fi";
...
const Table = ({ columns = [], data = [], onEdit, onView, onDelete, itemsPerPage = 10, extraActions,
    ...
    const handleExportExcel = useCallback(() => {
        ...
        const exportableColumns = columns.filter(col => !col.hideInExport && col.accessor !== 'actions');
        ...
        const exportData = sortedData.map((item, index) => {
            ...
            exportableColumns.forEach(col => {
                ...
                } else if (col.render && !col.accessor) { // Render only column
                    value = col.header; // Fallback or header
                } else {
                    value = item[col.accessor];

                    // Handle nested objects or arrays if needed (Basic string conversion)
                ...
                <div className="p-3 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-gray-100">
                    <div className="relative w-full md:w-72">
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-[4px] text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#228B22]/20 focus:border-[#228B22] transition-all text-sm placeholder-gray-400 font-jost"
                        />
                    ...
                    {allowExport && (
                        <div className="flex items-center gap-2 w-full md:w-auto">
                            <button
                                ...
                                className="flex items-center justify-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-[4px] hover:bg-emerald-100 transition-all font-medium text-sm w-full md:w-auto"
                            >
                                <FiDownload size={16} />
