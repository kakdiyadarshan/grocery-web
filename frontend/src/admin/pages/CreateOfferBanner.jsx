...
import { createOfferBanner, fetchOfferBanners, updateOfferBanner } from '../../redux/slice/offerbanner.slice';
...
const CreateOfferBanner = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        titleAnimationOut: 'none',
        subtitleAnimationOut: 'none',
        imageAnimation: 'none'
    });
    ...
    }, [dispatch, offerbanners.length]);

    useEffect(() => {
        if (id && offerbanners.length > 0) {
            const bannerToEdit = offerbanners.find(b => b._id === id);
            if (bannerToEdit) {
                ...
                setFormData({
                    ...
                    titleAnimation: bannerToEdit.titleAnimation || 'none',
                    subtitleAnimation: bannerToEdit.subtitleAnimation || 'none',
                    titleAnimationOut: bannerToEdit.titleAnimationOut || 'none',
                    subtitleAnimationOut: bannerToEdit.subtitleAnimationOut || 'none',
                    imageAnimation: bannerToEdit.imageAnimation || 'none'
                });
    ...
    const handleSubmit = useCallback(async (e) => {
        ...
        data.append('titleAnimation', formData.titleAnimation);
        data.append('subtitleAnimation', formData.subtitleAnimation);
        data.append('titleAnimationOut', formData.titleAnimationOut);
        data.append('subtitleAnimationOut', formData.subtitleAnimationOut);
        data.append('imageAnimation', formData.imageAnimation);
        ...
        try {
            if (currentBanner) {
                await dispatch(updateOfferBanner({ id: currentBanner._id, bannerData: data })).unwrap();
            } else {
                await dispatch(createOfferBanner(data)).unwrap();
            }
            navigate('/admin/offerbanners');
        } catch (error) {
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        value={formData.link}
                                        onChange={handleInputChange}
                                        className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-[4px] focus:border-primary outline-none transition-all placeholder:text-gray-400"
                                        placeholder="/shop"
                                    />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 border border-gray-200 p-1.5 rounded-[4px] bg-gray-50">
                                        <input
                                            ...
                                            className="h-8 w-8 rounded cursor-pointer border-0 p-0 bg-transparent"
                                        />
                                        <span className="text-xs text-gray-500 uppercase truncate">{formData.subtitleColor}</span>
                                    </div>
                                </div>
                                <div>
