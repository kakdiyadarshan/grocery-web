...
import { getAllShipping, saveAllShipping, uploadShippingImage } from '../../redux/slice/shipping.slice';
...
const ShippingPolicy = () => {
    ...
    const { loading, shipping } = useSelector((state) => state.shipping);
    ...
    const quillModules = useMemo(() => ({
        toolbar: {
            container: '#shipping-common-toolbar'
        },
        history: {
            delay: 1000,
            maxStack: 100,
    ...
    useEffect(() => {
        if (shipping && shipping.length > 0) {
            const shippingAsSections = shipping.map((policy) => ({
                id: policy._id || policy.id,
                type: policy.type || 'text',
                content: policy.description || '',
            }));
            setSections(shippingAsSections);
        }
    }, [shipping]);

    const handleSaveAll = useCallback(() => {
        const shippingData = sections.map(section => ({
            description: section.content,
            type: section.type
        }));
        dispatch(saveAllShipping(shippingData));
    }, [sections, dispatch]);
    ...
    return (
        <>
            <div className="flex justify-between items-center md:my-6 my-4 px-2 sm:px-0">
                <div>
                    <h2 className="text-2xl sm:text-3xl font-bold text-textPrimary">Shipping Policy</h2>
                </div>
                <button
            ...
            <div className="bg-white rounded-[4px] shadow-sm border border-gray-100 flex flex-col h-[calc(100vh-150px)] md:h-[calc(100vh-180px)] overflow-hidden">
                ...
                <div className="relative z-30 bg-white border-b border-gray-100 rounded-t-[4px] shadow-sm shrink-0">
                    ...
                    {/* Quill Toolbar */}
                    <div id="shipping-common-toolbar" className="p-2 sm:p-3 bg-white flex flex-wrap gap-1 md:gap-2 justify-center border-none">
                    ...
                    <div className="max-w-full mx-auto space-y-6 pb-20">
                        {(!sections || sections.length === 0) ? (
                            <div className="text-center py-16 text-gray-400 bg-white rounded-[4px] border border-dashed border-gray-200 shadow-sm transition-all hover:bg-gray-50 flex flex-col items-center">
                                <p className="text-base font-medium">No shipping policy sections found.</p>
                                            ...
                                            <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-200 rounded-[4px] bg-white relative min-h-[200px] transition-all hover:border-primary/30">
                                                ...
                                                {section.content ? (
                                                    <div className="mb-4 max-w-full overflow-hidden rounded-[4px] shadow-sm relative border border-gray-100">
                                                        <img src={section.content} alt="Shipping" className="max-w-full" />
                                                    </div>
                                                ) : (
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            if (file) {
                                                                ...
                                                                dispatch(uploadShippingImage(file)).then((action) => {
                                                                    if (action.payload) {
